...
<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
  {#each items as item}
    <a
      ...
      class="flex flex-col items-center justify-center p-8 rounded-2xl bg-white/5 border border-white/10 hover:border-red-600/50 hover:bg-white/[0.08] transition duration-300 group"
    >
      <div class="w-12 h-12 rounded-full bg-red-600/20 flex items-center justify-center text-red-500 mb-4 group-hover:scale-110 transition duration-300">
        <svg xmlns="http://www.w3.org/2000/svg" class="w-6 h-6" fill="currentColor" viewBox="0 0 24 24">
          <path d={getIconPath(item.icon)} />
        </svg>
      </div>
      <h4 class="text-white font-bold group-hover:text-red-500 transition duration-300">{item.name}</h4>
      <span class="text-xs text-gray-500 mt-2 truncate max-w-full">{item.url.replace('https://', '')}</span>
    </a>
  {/each}
</div>
